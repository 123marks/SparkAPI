...
type AccountRepository interface {
	...
	ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode string, createdFrom, createdTo *time.Time) ([]Account, *pagination.PaginationResult, error)
...
func (s *AccountService) Create(ctx context.Context, req CreateAccountRequest) (*Account, error) {
	...
	if account.Type == AccountTypeAPIKey && len(req.GroupIDs) > 0 {
		for _, gid := range req.GroupIDs {
			...
			if g.RequireOAuthOnly && (g.Platform == PlatformOpenAI || g.Platform == PlatformGrok || g.Platform == PlatformAntigravity || g.Platform == PlatformAnthropic || g.Platform == PlatformGemini || g.Platform == PlatformKiro) {

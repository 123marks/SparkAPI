export default {
  ...
  chatConsole: {
    ...
    requestUrlHint: '默认走当前 SparkAPI 网关；也可以填完整 OpenAI 兼容地址。',
    apiKey: '网关 API Key',
    apiKeyPlaceholder: 'sk-...',
    apiKeyHint: '只保存在当前浏览器会话中。请使用普通 SparkAPI Key，不要填管理密码。',
    manualKey: '手动输入密钥',
    savedKeysUnavailable: '暂时无法读取已保存 API Key，可手动输入。',
    model: '模型',
    temperature: '温度',
    mode: '模式',
    modes: {
    ...
    dropOverlayTitle: '松开后导入文件',
    dropOverlayDescription: '支持日志、配置、JSON、Markdown 和代码文本。',
    ...
    sending: '发送中...',
    stop: '停止',
    requestCancelled: '请求已停止。当前片段不会保存为完整回复。',
    requestTimedOut: '请求已超时。重试前请检查网关、代理和上游账号状态。',
    copyMessage: '复制消息',
    copied: '已复制',
    copyFailed: '复制失败',
    you: '你',
    ...
    loadedTools: '已加载',
    contextBudget: '{files} 文件 · {tools} 能力',
    ...
    imageSettings: {
      ...
      opaque: '不透明',
      transparent: '透明',
      hint: '使用 OpenAI 兼容图片端点。GPT image 默认返回 base64；URL、透明背景、审核强度、输出格式和压缩会按所选模型兼容性自动透传或降级。发送前请确认所选 Key 已绑定支持图片生成和图片计费的分组。'
    },
    ...
    activity: {
      ...
      streamingWithChars: '正在输出 · {chars} 字符',
      progressLabel: '请求进度',
      defaultHint: '浏览器正在等待网关打开上游请求。',
      waitingHint: '还没有收到文本。通常是上游模型仍在思考，或供应商返回了非流式响应。',
      streamingHint: '文本正在逐步返回，系统仍会做安全渲染和格式整理。',
      streamingEmptyHint: '响应流已打开，但还没有收到文本片段。',
      imageHint: '图片生成通常比对话更慢。网关等待上游任务时请保持页面打开。'
    },
    tools: {
      title: '能力目录',
  ...
  admin: {
    ...
    accounts: {
      ...
      dataActions: '数据操作',
      toolActions: '工具',
      kiroSidecarTitle: 'Kiro-RS 管理面板',
      kiroSidecarDesc: '检查可选 Kiro-RS 侧车状态，并打开它自己的账号管理面板。',
      kiroSidecarChecking: '正在检查 Kiro-RS 侧车...',
      kiroSidecarHealthy: 'Kiro-RS 可访问',
      kiroSidecarUnhealthy: 'Kiro-RS 需要处理',
      kiroSidecarNotChecked: '打开或刷新此面板即可检查侧车状态。',
      kiroSidecarAdminUrl: '管理面板地址',
      kiroSidecarOpenAdmin: '打开 Kiro-RS 管理面板',
      kiroSidecarSparkBaseUrl: 'SparkAPI 账号 Base URL',
      kiroSidecarBaseUrlHint: '当 SparkAPI 与 Kiro-RS 在同一个 Docker 网络中运行时，在 SparkAPI 的 Anthropic API Key 上游里填写这个地址。',
      kiroSidecarInternalUrl: '容器内部地址',
      kiroSidecarApiKeyConfigured: 'Client Key 已配置',
      kiroSidecarLoadFailed: '加载 Kiro-RS 侧车状态失败',
      kiroSidecarModelCount: '{count} 个模型',
      viewColumns: '列显示',
      ...
      dataImportWarning: '导入将创建新账号与代理，开始前可以给导入账号统一绑定分组。',
      dataImportGroupBindingHint: '选中的分组会追加到每个导入账号，文件中已有的 group_ids 会保留。',
      dataImportSkipDefaultGroup: '未选分组时跳过默认分组绑定',
      dataImportSkipDefaultGroupHint: '保持导入账号干净，便于后续手动分组管理。',
      dataImportFile: '数据文件',
      dataImportFileHint: '可选择多个 JSON 文件，支持导出原文、data 字段包装或文件内数组',
      dataImportSelectedFiles: '已选择 {count} 个文件：{names}',
      dataImportSelectedSize: '总大小：{size}',
      dataImportFileTooLarge: '{name} 文件过大，单个文件不能超过 {max}。',
      dataImportTotalTooLarge: '选中文件总大小过大，一次导入不能超过 {max}。',
      ...
      createdRangeAll: '全部添加时间',
      createdRangeToday: '今天添加',
      createdRange7d: '7 天内添加',
      createdRange30d: '30 天内添加',
      createdRangeCustom: '自定义添加时间',
      createdFrom: '添加开始时间',
      createdTo: '添加结束时间',
      oauthType: 'OAuth',
      // Schedulable toggle
      ...
      bulkActions: {
        ...
        selectedHint: '这里的操作仅作用于当前页已选中的账号。',
        filteredReady: '未选中账号',
        filteredHint: '按当前筛选结果进行批量更新。',
    ...
    settings: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 SparkAPI 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 SparkAPI 用户登录',
  ...
  onboarding: {
    ...
    admin: {
      welcome: {
        title: '👋 欢迎使用 SparkAPI',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">SparkAPI 是一个强大的 AI 服务中转平台，让您轻松管理和分发 AI 服务。</p><p style="margin-bottom: 12px;"><b>🎯 核心功能：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>📦 <b>分组管理</b> - 创建不同的服务套餐（VIP、免费试用等）</li><li>🔗 <b>账号池</b> - 连接多个上游 AI 服务商账号</li><li>🔑 <b>密钥分发</b> - 为用户生成独立的 API Key</li><li>💰 <b>计费管理</b> - 灵活的费率和配额控制</li></ul><p style="color: #10b981; font-weight: 600;">接下来，我们将用 3 分钟带您完成首次配置 →</p></div>',
        nextBtn: '开始配置 🚀',
        prevBtn: '跳过'
      },
      groupManage: {
        title: '📦 第一步：分组管理',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;"><b>什么是分组？</b></p><p style="margin-bottom: 12px;">分组是 SparkAPI 的核心概念，它就像一个"服务套餐"：</p><ul style="margin-left: 20px; margin-bottom: 12px; font-size: 13px;"><li>🎯 每个分组可以包含多个上游账号</li><li>💰 每个分组有独立的计费倍率</li><li>👥 可以设置为公开或专属分组</li></ul><p style="margin-top: 12px; padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>💡 示例：</b>您可以创建"VIP专线"（高倍率）和"免费试用"（低倍率）两个分组</p><p style="margin-top: 16px; color: #10b981; font-weight: 600;">👉 点击左侧的"分组管理"开始</p></div>'
      },
      createGroup: {
        title: '➕ 创建新分组',
        description:
    ...
    user: {
      welcome: {
        title: '👋 欢迎使用 SparkAPI',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 SparkAPI AI 服务平台。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 创建 API 密钥</li><li>📋 复制密钥到您的应用</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: '开始 🚀',
        prevBtn: '跳过'
      },
      keyManage: {

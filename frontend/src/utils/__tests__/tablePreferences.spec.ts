...
describe('tablePreferences', () => {
  ...
  it('merges configured options with SparkAPI large-page defaults', () => {
    ...
    expect(getConfiguredTablePageSizeOptions()).toEqual([10, 20, 50, 100, 200, 500, 1000])
  })
  ...
  it('keeps SparkAPI built-in selectable defaults when only default size is configured', () => {
    window.__APP_CONFIG__ = {
      table_default_page_size: 1000
    } as any
  ...
  it('upgrades legacy Sub2API page-size config that only allowed up to 50', () => {
    window.__APP_CONFIG__ = {
      table_default_page_size: 50,
